...
SELECT  C.NOME, IFNULL(C.EMAIL,'NAO INFORMADO') AS 'EMAIL',
        T.NUMERO AS CELULAR
FROM CLIENTE C
INNER JOIN ENDERECO E
ON C.IDCLIENTE = E.ID_CLIENTE
...
SELECT  C.NOME, C.SEXO, IFNULL(C.EMAIL,'NAO INFORMADO') AS 'EMAIL', 
		T.TIPO, T.NUMERO,
		E.BAIRRO, E.CIDADE, E.ESTADO
FROM CLIENTE C 
INNER JOIN TELEFONE T 
...
/* Aggregating functions (using TRUNCATE) */
...
SELECT 	SUM(JANEIRO) AS TOTAL_JAN,
		SUM(FEVEREIRO) AS TOTAL_FEV,
		SUM(MARCO) AS TOTAL_MAR,
		SUM(JANEIRO + FEVEREIRO + MARCO) AS 'TOTAL_TRI'
FROM VENDEDORES;
...
SELECT 	NOME,
		...
		(JANEIRO + FEVEREIRO + MARCO) AS 'GROSS TOTAL',
		(JANEIRO + FEVEREIRO + MARCO) * .15 AS '% OFF',
		((JANEIRO + FEVEREIRO + MARCO)-((JANEIRO + FEVEREIRO + MARCO) * .15)) AS 'REAL TOTAL'
FROM VENDEDORES;
